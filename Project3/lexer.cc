...
//Fill the tokenlist in the constructor
LexicalAnalyzer::LexicalAnalyzer()
{
    ...
    //keep populating tokenList until EOF
...
bool LexicalAnalyzer::SkipSpace()
{
    ...
    //Get charcater from inputbuffer
    input.GetChar(c);

    //increment line number on newline
    line_no += (c == '\n');

    //while file has not ended and is a space, skip spaces until next char
    //loop to skip space in line
    while (!input.EndOfInput() && isspace(c)) {
        space_encountered = true;
        input.GetChar(c);
        //increment line number on end of line
        line_no += (c == '\n');
    }

    //unget non-space char
    if (!input.EndOfInput()) {
        input.UngetChar(c);
    }
...
Token LexicalAnalyzer::GetTokenMain()
{
    ...
    switch (c) {
        ...
        case '!':   tmp.token_type = END_OF_FILE;   return tmp;
        case '<':
            input.GetChar(c);
            if (c == '>') {
